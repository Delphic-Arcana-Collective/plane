...
async function discoverProjects(page: Page): Promise<[ProjectRef, ProjectRef]> {
  const pinnedA = process.env.PROJECT_A_ID;
  const pinnedB = process.env.PROJECT_B_ID;
  if (pinnedA && pinnedB) {
    return [
      { id: pinnedA, name: process.env.PROJECT_A_NAME ?? pinnedA },
      { id: pinnedB, name: process.env.PROJECT_B_NAME ?? pinnedB },
    ];
  }
...
async function waitForProjectReady(page: Page, projectId: string, timeoutMs: number) {
  ...
  while (Date.now() < deadline) {
    ...
    // Requirement: never treat empty list as ready; both header and rows must be > 0.
    const ready = state.headerCount > 0 && state.issueBlocks > 0;
...
async function assertProjectView(page: Page, label: string) {
  ...
  if (state.headerCount <= 0 || state.issueBlocks <= 0) {
    throw new Error(
      `${label}: expected header>0 and rows>0, got header=${state.headerCount} rows=${state.issueBlocks}`
    );
  }
}

async function runRound(page: Page, projectA: ProjectRef, projectB: ProjectRef, round: number, workerId: number) {
  console.log(`[prod-stress] w${workerId} r${round} -> A`);
  await clickProject(page, projectA);
  await waitForProjectReady(page, projectA.id, 45_000);
  await assertProjectView(page, `w${workerId} r${round} project A`);

  console.log(`[prod-stress] w${workerId} r${round} -> B`);
  await clickProject(page, projectB);
  await waitForProjectReady(page, projectB.id, 45_000);
  await assertProjectView(page, `w${workerId} r${round} project B`);

  console.log(`[prod-stress] w${workerId} r${round} -> A (back)`);
  await clickProject(page, projectA);
  await waitForProjectReady(page, projectA.id, 45_000);
  await assertProjectView(page, `w${workerId} r${round} back to A`);
  console.log(`[prod-stress] w${workerId} r${round} ok`);
}
...
async function bootProject(page: Page, project: ProjectRef) {
  ...
  await waitForProjectReady(page, project.id, 45_000);
}
...
async function main() {
  ...
  try {
    ...
    console.log(`[prod-stress] projects: A="${projectA.name}" (${projectA.id}) B="${projectB.name}" (${projectB.id})`);
