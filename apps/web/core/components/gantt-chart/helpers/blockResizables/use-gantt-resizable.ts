...
export const useGanttResizable = (
  ...
  /** True only after pointer moved during this drag — plain clicks must not POST dates. */
  ...
  const handleBlockDrag = (
    ...
    initialPositionRef.current = {
      ...
      offsetX: startMouseX - block.position.marginLeft,
    };
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      const currentMouseX = moveEvent.clientX - containerLeft - SIDEBAR_WIDTH + ganttContainerElement.scrollLeft;
    ...
    const handleMouseUp = () => {
      ...
      // Click-to-open must not hit the dates API (BFF/Plane toast noise).
